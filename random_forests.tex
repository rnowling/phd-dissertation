...
\noindent where $\bar{\Delta}_{f, t}$ is the mean decrease in impurity of feature $f$ for tree $t$ and $N_t$ is the total number of trees.
...
\begin{table}[H]
  ...
  \tablenote{Examples of two groups with known and unknown genotypes, before and after imputation with a threshold frequency of 100\%.}
\end{table}
...
\begin{table}[H]
  ...
  \tablenote{Examples of SNP values for two individuals.  The SNPs are labeled by pairs of chromosome and position.}
\end{table}
...
\begin{table}[H]
  ...
  \tablenote{Feature matrix for the SNPs of two individuals.  The features are labeled by triplets of chromosome, position, and genotype.}
\end{table}
